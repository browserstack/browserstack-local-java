...
public class BrowserStackTunnel {
	...
	public BrowserStackTunnel(String key, File binaryPath) throws BrowserStackTunnelException {
	  if(!binaryPath.exists()) {
        ...
        throw new BrowserStackTunnelException(
            "The binary Path " + binaryPath.getPath() +
  ...
  public void addArgs(String key, boolean value) {
    key = key.trim().toLowerCase();
    ...
    this.browserStackLocal.run(listener);
	}
    ...
    try {
      ...
      throw new BrowserStackTunnelException("Cannot reach browserstack.com. Please check your internet connection.", e);
    }
    try {
    ...
    try {
      ...
      throw new BrowserStackTunnelException(
          "Zip extraction failed. The downloaded file might be corrupt.", e);
    }
    ...
    try {
      ...
      throw new BrowserStackTunnelException(
          "Cannot create or write to " + targetFile.getAbsoluteFile() + ". Please check the folder permissions.", e);
    }
